...
export function getGroupPlanCame(groupId: number) {
  return request<import('../types').CameSummary>(`/groups/${groupId}/plan/formulation/came`)
}

export function saveGroupPlanCame(groupId: number, payload: import('../types').UpdateCamePayload) {
  return request<import('../types').CameSummary>(`/groups/${groupId}/plan/formulation/came`, {
    method: 'PUT',
    body: JSON.stringify(payload),
  })
